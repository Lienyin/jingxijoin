...
public class Area2Adapter extends BaseAdapter {
    ...
    private List<AreaEntity.Citys> list;

    public Area2Adapter(Context context){
        this.context=context;
    }

    public void setData(List<AreaEntity.Citys> list){
        this.list = list;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null){
            ...
            convertView = LayoutInflater.from(context).inflate(R.layout.stie_plus2_adapter,null);
            holder.tv_site_name = convertView.findViewById(R.id.tv_site_name);
            convertView.setTag(holder);
        }else{
            holder = (ViewHolder) convertView.getTag();
        }
        AreaEntity.Citys data = list.get(position);
